import {
  searchArtfaktaTaxon,
  fetchArtfaktaSpeciesData,
} from "../api/artfakta.ts";
...
export async function fetchSpeciesInfo(species: Map<number, Species>): Promise<{
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    const gridStatus = gridData.has(sp.id)
      ? `${gridData.get(sp.id)!.length} cells`
      : "no grid";
    console.log(` ${infoStatus}, ${gridStatus}`);
  }

  console.log(
    `  ${speciesInfo.size} species with info, ${gridData.size} with grid data`,
  );

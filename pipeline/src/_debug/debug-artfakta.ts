...
for (const zoom of [7, 9, 10, 11]) {
  ...
  const totalObs = cells.reduce(
    (sum: number, c: any) => sum + c.observationsCount,
    0,
  );
  const maxObs =
    cells.length > 0
      ? Math.max(...cells.map((c: any) => c.observationsCount))
      : 0;
  ...
  if (cells.length > 0) {
    ...
    const latDiff = Math.abs(
      c.boundingBox.topLeft.latitude - c.boundingBox.bottomRight.latitude,
    );
    const lngDiff = Math.abs(
      c.boundingBox.bottomRight.longitude - c.boundingBox.topLeft.longitude,
    );

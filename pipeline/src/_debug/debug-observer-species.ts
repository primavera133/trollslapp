...
import { ADB_KEY, CURRENT_YEAR, SOS_BASE_URL, START_YEAR } from "../config.ts";
...
console.log(
  `Fetching year by year for "${OBSERVER}" in province "${PROVINCE_FEATURE_ID}"...`,
);
...
const species = [...speciesMap.entries()].sort((a, b) =>
...
if (secondary.length > 0) {
  ...
  for (const [id, s] of secondary) {
    console.log(
      `  [${id}] ${s.swedish ?? s.scientific} (${s.firstDate.slice(0, 10)})`,
    );
...
for (const [id, s] of species) {
  ...
  console.log(
    `${marker} ${s.firstDate.slice(0, 10)}  ${s.swedish ?? s.scientific}`,
  );
}

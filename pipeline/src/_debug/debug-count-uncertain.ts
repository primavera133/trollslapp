...
console.log(
  `${"Year".padEnd(6)} ${"All".padStart(8)} ${"Certain".padStart(8)} ${"Uncertain".padStart(10)} ${"% uncertain".padStart(12)}`,
);
...
for (let year = START_YEAR; year <= CURRENT_YEAR; year++) {
  ...
  const [resAll, resCertain] = await Promise.all([
    ...
    fetch(`${SOS_BASE_URL}/Observations/Count`, {
      ...
      body: JSON.stringify({
        ...base,
        taxon: { ...base.taxon, isUncertain: false },
      }),
    }),
  ]);
  ...
  if (!resAll.ok || !resCertain.ok) {
    console.log(
      `${year}: count failed (${resAll.status} / ${resCertain.status})`,
    );
    continue;
  }
  ...
  console.log(
    `${String(year).padEnd(6)} ${all.toLocaleString().padStart(8)} ${certain.toLocaleString().padStart(8)} ${uncertain.toLocaleString().padStart(10)} ${(pct + "%").padStart(12)}`,
  );
...
const totalPct =
  totalAll > 0 ? ((totalUncertain / totalAll) * 100).toFixed(1) : "0.0";
console.log(
  `${"TOTAL".padEnd(6)} ${totalAll.toLocaleString().padStart(8)} ${totalCertain.toLocaleString().padStart(8)} ${totalUncertain.toLocaleString().padStart(10)} ${(totalPct + "%").padStart(12)}`,
);

...
import { countObservations, fetchAllObservations } from "../api/sos.ts";
import { ADB_KEY } from "../config.ts";
...
const juneObs = await fetchAllObservations(juneFilter, (msg) =>
  console.log(`  ${msg}`),
);
...
for (const obs of target) {
  ...
  console.log(
    `    uncertainIdentification: ${obs.identification?.uncertainIdentification}`,
  );
}
...
const directObs = await fetchAllObservations(directFilter, (msg) =>
  console.log(`  ${msg}`),
);
const directSkane = directObs.filter(
  (o) => o.location.province?.featureId === "1",
);
...
const juneObs2 = await fetchAllObservations(juneFilter, (msg) =>
  console.log(`  ${msg}`),
);

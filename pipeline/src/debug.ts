...
import { ADB_KEY, DYNTAXA_KEY, SOS_BASE_URL, DYNTAXA_BASE_URL, TAXON_GROUPS } from './config.ts'
...
if (r1.ok) {
  ...
  const ids = Array.isArray(raw) ? raw as number[] : (raw as any)?.taxonIds ?? (raw as any)?.ids ?? (raw as any)?.childTaxonIds ?? []

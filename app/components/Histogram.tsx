...
export function Histogram({
  ...
  const maxCount = useMemo(() => {
    ...
    const yearMax = selectedYear
      ? Math.max(0, ...selectedYear.map((d) => d.total))
      : 0;
  ...
  const peakWeek = allYears.reduce(
  ...
  return (
    ...
    >
      ...
      {monthLabels.map(({ week, label }) => {
        ...
        return (
          <SvgText key={label} x={x} y={height - 6} fontSize={9} fill="#717171">
            {label}
          </SvgText>
        );
      })}
